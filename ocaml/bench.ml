...
let bench_scheduler_multicore iters =
  let num_fibers = 40 and m = iters / 40 and num_workers = 4 in
...
let () =
  ...
  bench "perform dispatch (sched, 1W)"    100_000   bench_scheduler;
  bench "perform dispatch (sched, 4W)"    100_000   bench_scheduler_multicore;
  Printf.printf "\n"
